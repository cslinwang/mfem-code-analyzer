...
def monitor_cpu(threshold=80, interval=1):
    """
    循环监控CPU利用率，当CPU利用率低于指定阈值时停止。

    :param threshold: CPU利用率的阈值，当CPU利用率低于这个值时函数将返回。
    :param interval: 读取CPU利用率的时间间隔（秒）。
    :return: CPU利用率低于阈值时的利用率。
    """
    while True:
        cpu_usage = psutil.cpu_percent(interval=interval)
        print(f"Current CPU usage: {cpu_usage}%")
        if cpu_usage < threshold:
            print(f"CPU usage dropped below {threshold}%. Stopping monitor.")
            return cpu_usage
        else:
            print(f"CPU usage is above {threshold}%.")
