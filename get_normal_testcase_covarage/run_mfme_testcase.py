...
class MFEMIssueProcessor:
    ...
    def pre_issue_file(self, issue_name, issue_data, testcase_path):
        # 获取Git SHA for fix
        fix_sha = issue_data.get("fix_sha", "")
        self.switch_version(fix_sha)
    ...
    def copy_testcase(self, name, testcase):
        source_path = os.path.join(
            self.bugs_path, name, testcase.split("/")[-1])
        ...
        if testname.startswith("ex"):
            ...
            try:
                ...
                result = subprocess.run(
                    ["/root/mfem-code-analyzer/get_normal_testcase_covarage/run_exp_tesscase.sh", testname], capture_output=True, text=True)
                # 将标准输出和错误输出记录到日志
                print(result.stdout)
            ...
            try:
                # 使用capture_output来捕获输出
                result = subprocess.run(
                    ["/root/mfem-code-analyzer/get_normal_testcase_covarage/run_tesscase.sh", testname], capture_output=True, text=True)
                # 将标准输出和错误输出记录到日志
                logger.info(f"Running test case: {testname}")
                logger.info(f"CWD: {cmd_path}")
                logger.info(result.stdout)
